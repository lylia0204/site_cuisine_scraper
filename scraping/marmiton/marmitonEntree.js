...
const getDataFromUrl = async (browser, url) => {
    ...
    return page.evaluate(() => {
        ...
        if (tpsPreparationPur != null) {
            tpsPreparationPur = tpsPreparationPur.innerText
        }
        tpsPreparationSansPrep = tpsPreparationPur.replace(/Préparation/, '' )
        let tpsPreparation= tpsPreparationSansPrep.replace(/:/,'')
        

        let tpsTotalPur = document.querySelector('div.recipe-infos__timmings__total-time ')
        if (tpsTotalPur != null) {
            tpsTotalPur = tpsTotalPur.innerText
        }
        tpsTotalSansEspace = tpsTotalPur.replace(/ /, '' )
        let tpsTotal= tpsTotalSansEspace.replace(/TempsTotal:/, '')  
